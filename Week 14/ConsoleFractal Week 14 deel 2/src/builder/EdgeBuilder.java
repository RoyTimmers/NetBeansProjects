...
public class EdgeBuilder implements Observer {
    ...
    private void MMFSave(Object e) throws IOException {
        ...
        RandomAccessFile ras = new RandomAccessFile("C:\\Users\\royti\\OneDrive\\Documenten\\NetBeansProjects\\Week 14\\Edges\\edge.ras", "rw");
        ...
        if (first) {
            ...
            ELock = fc.lock(0, 4 + ((long) (3 * Math.pow(4, kf.getLevel())) * 42), false);
            out.putInt(kf.getLevel());
            first = false;
        } else {
            ELock = fc.lock(edgeCount * 42 + 4, ((long) (3 * Math.pow(4, kf.getLevel())) * 42) - (edgeCount * 42 + 4), false);
        }
        ...
        if (first) {
            ELock.release();
            ELock = fc.lock(46, ((long) (3 * Math.pow(4, kf.getLevel())) * 42), false);
        } else {
            ELock.release();
            ELock = fc.lock(edgeCount * 42 + 4, ((long) (3 * Math.pow(4, kf.getLevel())) * 42) - (edgeCount * 42 + 4), false);
        }
